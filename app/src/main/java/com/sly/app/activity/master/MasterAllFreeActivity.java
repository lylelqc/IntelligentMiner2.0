...
public class MasterAllFreeActivity extends BaseActivity implements ICommonViewUi {
    ...
    @BindView(R.id.tv_master_free_month)
    TextView tvMasterFreeMonth;

    @BindView(R.id.tv_master_date)
    TextView tvMasterDate;
    @BindView(R.id.tv_master_free_day)
    TextView tvMasterFreeDay;
    @BindView(R.id.tv_master_free_manage)
    TextView tvMasterFreeManage;
    @BindView(R.id.tv_master_free_repair)
    TextView tvMasterFreeRepair;
    ...
    private List<MasterAllFreeBean> mResultList = new ArrayList<>();
    ...
    protected int getContentViewLayoutID() {
        return R.layout.activity_master_all_free;
    }
    ...
    public void toRequest(int eventTag) {
        ...
        map.put("Rounter", NetConstant.GET_MASTER_MONTH_FREE);
        map.put("Token", Token);
    ...
    public void getRequestData(int eventTag, String result) {
        ...
        if (eventTag == NetConstant.EventTags.GET_MASTER_MONTH_FREE) {
            mResultList.clear();
    ...
    private void refreshListView(int position, boolean isDecase) {
        MasterAllFreeBean bean = mResultList.get(isDecase ? position -1 : position);
        tvMasterFreeMonth.setText((bean.getRepairSum()+bean.getPowerSum())+"");
        tvMasterDate.setText(bean.getDate().split(" ")[0]);
        tvMasterFreeDay.setText("+" + (bean.getDateRepairSum()+bean.getDatePowerSum()));
        tvMasterFreeManage.setText(bean.getDatePowerSum()+"");
        tvMasterFreeRepair.setText(bean.getDateRepairSum()+"");
    }
    ...
    private void initBarChart() {
        ...
        List<String> label = Arrays.asList(
                getString(R.string.master_free_manage), getString(R.string.master_free_repair));
        List<Integer> colors = Arrays.asList(
                getResources().getColor(R.color.sly_navigation_bar), getResources().getColor(R.color.sly_pic_dddddd));

        for (MasterAllFreeBean bean : mResultList) {
            xValues.add((float)bean.getDay());
            yValue1.add((float) bean.getDatePowerSum());
            yValue2.add((float) bean.getDateRepairSum());
        }

        chartDataList.add(yValue1);
        chartDataList.add(yValue2);

        barUtil = new BarUtil(bcFreePic);
        barUtil.initCalPowerPic(this, bcFreePic, mResultList, true);
        barUtil.showBarChart(xValues, chartDataList, label, colors);
