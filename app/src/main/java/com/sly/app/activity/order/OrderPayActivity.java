...
public class OrderPayActivity extends BaseActivity {
    ...
    protected void initView() {
        ...
        if (!MallType.isEmpty() && MallType.equals("DK")) {
            mTvWalletName.setText("微商库存额度");
            mTvWalletNameTag.setText("微商城产品使用库存额度兑换");
            mRlPayWeChat.setVisibility(View.GONE);
            mRlPayAliPay.setVisibility(View.GONE);
        } else if (!MallType.isEmpty() && MallType.equals(Constants.MAIL_TYPE_WS)) {
            mTvWalletName.setText("产品券");
            mTvWalletNameTag.setText("商品使用产品券兑换");
            mRlPayWeChat.setVisibility(View.GONE);
            mRlPayAliPay.setVisibility(View.GONE);
        }
...
/*                if (CommonUtils.isBlank(PayType)) {
            ...
            case R.id.rl_pay_wallet_ticket:
                if (isPwd == true) {
                    ...
                    Intent intent = new Intent();
                    intent.putExtra("pay", "pay");
                    intent.setClass(OrderPayActivity.this, UpDataPwActivity.class);
                    startActivity(intent);
                }
                break;
            ...
            public boolean onSuccess(int statusCode, Headers headers, String content) {
                ...
                if (mReturnBean.getStatus().equals("1")) {
                    ...
                    Runnable payRunnable = new Runnable() {
                        @Override
                        public void run() {
                            PayTask PayInfo = new PayTask(OrderPayActivity.this);
...
//        confirmButton.setBackgroundColor(Color.parseColor("#ffffff"));
    ...
    private void ProOrderPay(String memberCode, String token, final String orderNo, String orderPrice, String mallType, String passWord) {
        ...
        map.put("Token", token);
        map.put("OrderNo", orderNo);
        //   map.put("PurseCode", memberCode);
        map.put("PayPassword", passWord);
        if (mallType.equals(Constants.MAIL_TYPE_WS)) {
            map.put("CurrencyCode", "Product");
        } else if (mallType.equals("DK")) {
            map.put("CurrencyCode", "DistributionShop");
        } else if (mallType.equals("V")) {
            map.put("CurrencyCode", "YB");
        } else {
            ToastUtils.showToast("缺少商城类型");
        }
        final String json = CommonUtils.GsonToJson(map);
