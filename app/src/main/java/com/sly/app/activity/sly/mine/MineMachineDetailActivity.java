...
public class MineMachineDetailActivity extends BaseActivity {
    ...
    public void toRequest(final int eventType) {
        ...
        if(LoginType.equals("Miner")){
            map.put("Rounter", NetWorkCons.GET_MANAGE_MACHINE_MINE_DETAILS_HEAR);
        ...
        if(eventType == NetWorkCons.EventTags.GET_MANAGE_MACHINE_DETAILS_FOOT){
        ...
        HttpClient.postJson(NetWorkCons.BASE_URL, json, new HttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, String content) {
                ...
                if(eventType == NetWorkCons.EventTags.GET_MANAGE_MACHINE_DETAILS_HEAR
                        || eventType == NetWorkCons.EventTags.GET_MANAGE_MACHINE_MINE_DETAILS_HEAR ){
                        ...
                        if(mReturnBean.getData().getRows().size() > 0){
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if(LoginType.equals("Miner")){
                                        tvMd4.setText("所属矿场");
                                        tvBelongArea.setText(mHeaderList.get(0).getMine01_MineName());
                                    }else{
                    ...
                    if(mReturnBean.getStatus().equals("1") ){
                        if(mReturnBean.getData().getRows().size() > 0){
                            ...
                            int color = Color.parseColor("#D3F2F2");
                            showLineChart(mPicDataList, LcSlyBoDong, color, color);
                        }
                    }
                }
            }
...
//        leftYAxis.enableGridDashedLine(10f, 0f, 0f);
        ...
        leftYAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                return Math.round((value / 1024)) + "T±5% ";
            }
        });
...
//        legend.setEnabled(false);
     ...
     * @param bgColor    背景颜色
     */
    public static void showLineChart(List<MDetailsPicDataBean> dataList, LineChart lineChart, int lineColor, int bgColor) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            MDetailsPicDataBean data = dataList.get(i);
            float yValue = (float) data.getMine63_NowCalcForce();
            /**
