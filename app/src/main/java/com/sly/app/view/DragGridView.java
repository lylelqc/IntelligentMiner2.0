...
import com.sly.app.model.sly.SelectImageModel;
...
public class DragGridView extends GridView {
    ...
    private OnItemLongClickListener onLongClickListener = new OnItemLongClickListener(){
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if(((DragGridAdapter)getAdapter()).getDragType(position) == SelectImageModel.ADD_IMAGE_TYPE){
                return true;
            }
            ((DragGridAdapter)getAdapter()).dragStatus(true);
            //记录长按item位置
            ...
            //设置被长按item不显示
            ((DragGridAdapter)getAdapter()).hideView(position);
            return true;
        }
    };
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        //dragImageView处于被拖动时，更新dragImageView位置
            ...
            if((currDraggedPosition != AdapterView.INVALID_POSITION) && (currDraggedPosition != preDraggedOverPositon)) {
                ((DragGridAdapter)getAdapter()).swapView(preDraggedOverPositon, currDraggedPosition);
                preDraggedOverPositon = currDraggedPosition;
            }
        }
        //释放dragImageView
        else if((ev.getAction() == MotionEvent.ACTION_UP) && (isViewOnDrag == true)) {
            ((DragGridAdapter)getAdapter()).showHideView();
            ((DragGridAdapter)getAdapter()).dragStatus(false);
            if((int)dragImageView.getTag() == DRAG_IMG_SHOW) {
                windowManager.removeView(dragImageView);
                dragImageView.setTag(DRAG_IMG_NOT_SHOW);
            }
    ...
    public ListGridView.OnTouchInvalidPositionListener mTouchInvalidPosListener;
    /**
     * 点击空白区域时的响应和处理接口
     */
    public void setOnTouchInvalidPositionListener(
            ListGridView.OnTouchInvalidPositionListener listener) {
