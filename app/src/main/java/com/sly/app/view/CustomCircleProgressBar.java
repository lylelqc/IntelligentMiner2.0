...
public class CustomCircleProgressBar extends View {
    ...
    private int progressBarTextColor;   //圆环内文字颜色
    private float progressBarTextSize;    //圆环内文字大小
    ...
    public CustomCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        outsideRadius = a.getDimension(R.styleable.CustomCircleProgressBar_outside_radius, AppUtils.dp2px(getContext(), 60.0f));
        insideColor = a.getColor(R.styleable.CustomCircleProgressBar_inside_color, ContextCompat.getColor(getContext(), R.color.sly_btn_blue));
        ...
        progressTextSize = a.getDimension(R.styleable.CustomCircleProgressBar_progress_text_size, AppUtils.dp2px(getContext(), 14.0f));
        progressBarTextColor = a.getColor(R.styleable.CustomCircleProgressBar_progressbar_text_color, ContextCompat.getColor(getContext(), R.color.sly_text_999999));
        progressBarTextSize = a.getDimension(R.styleable.CustomCircleProgressBar_progressbar_text_size, AppUtils.dp2px(getContext(), 7.0f));
        progressWidth = a.getDimension(R.styleable.CustomCircleProgressBar_progress_width, AppUtils.dp2px(getContext(), 10.0f));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setStrokeCap(Paint.Cap.ROUND); //设置进度两头圆弧
        ...
        canvas.drawText(progressText, getMeasuredWidth() / 2 - rect.width() / 2 - rect.width()/4, baseline, paint);

        //第四步:画圆环内百分比
        rect2 = new Rect();
        paint.setColor(progressBarTextColor);
        paint.setTextSize(progressBarTextSize);
        paint.setStrokeWidth(0);
        paint.getTextBounds("%", 0, "%".length(), rect);
        canvas.drawText("%", getMeasuredWidth() / 2 + rect.width() , baseline, paint);
    }
    ...
    private String getProgressText() {
        return  isInteger ? (int)((progress / maxProgress) * 100) +"" : ((progress / maxProgress) * 100) +"";
    }
